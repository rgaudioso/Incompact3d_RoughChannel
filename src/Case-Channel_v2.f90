...
!
  ...
  subroutine geomcomplex_channel(epsi,nxi,nxf,ny,nyi,nyf,nzi,nzf,dxx,yp,dzz,remp)
    ...
    if (isurf==1) then
       ...
       !!$     ys(ks,is) = A * cos(om*xm) * cos(om*zm) + 2.*A
       !!$  enddo
       !!$enddo
       !!$----------------------------------------------------
       lambda = 7.07*ampl !MacDonald
       !lambda = 2.*7.07*ampl !MacDonald
       om = (2.*pi)/lambda
       !do ks = 1,nz
       !   zm = zp(ks)
       !  do is = 1,nx
       ...
       do ks = nzi,nzf !loop in global indices
         ...
         do is = nxi,nxf !loop in global indices
            xm = real(is-1, mytype)*dxx
            ys(ks,is) = ampl * cos(om*xm) * cos(om*zm) + 5.*ampl
            !ys(ks,is) = ampl * cos(om*xm) * cos(om*zm) + ampl
         enddo
       enddo
       do k=nzi,nzf
         do j=nyi,nyf
           ...
           do i=nxi,nxf
              if ((ym.le.(ys(k,i))).or.(ym.ge.(yly-ys(k,i)))) then
                 epsi(i,j,k)=remp
                 !!$!====DEBUG
                 !!$!if (dxx.eq.dx.and.dz.eq.dzz) then
                 !!$if (dxx.ne.dx) then
                 !!$    if (k.eq.nz/2+1) print*,"eps=", real(i-1, mytype)*dxx,&
                 !!$                                    ym,&
                 !!$                                    real(k-1, mytype)*dzz,&
                 !!$                                    real(epsi(i,j,k),4)
                 !!$endif                                           

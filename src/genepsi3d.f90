  ...
  subroutine geomcomplex(epsi, nx, nxi, nxf, ny, nyi, nyf, nz, nzi, nzf, xp, yp, zp, remp)
    ...
    ELSEIF (itype.EQ.itype_rough) THEN
       ...
       if (ialloc) then
           ...
           CALL read_roughness(rmat,nx,nz)
           ialloc=.false.
       endif
       !stop
...
! #############################################################################
  subroutine geomcomplex_io(nx,ny,nz,ep1,nobjx,nobjy,nobjz,xi,xf,yi,yf,zi,zf,&
    ...
    !open(67,file='data/geometry/epsi.dat',form='formatted',access='direct',recl=13)
    if (nrank==0) then
        write(*,"('epsi xsize1               : ',I17)") xsize(1)
        write(*,"('epsi xsize2               : ',I17)") xsize(2)
        write(*,"('epsi xsize3               : ',I17)") xsize(3)
    ...
    !   do j=1,xsize(2)
    !      do i=1,xsize(1)
    !         count = (k*j-1)*xsize(1)+i
    !         if (read_flag) then
    !            read(67,'(E8.2,A)',rec=count) ep1(i,j,k),tmp_char
    !         else
    !            write(67,'(E8.2,A)',rec=count) ep1(i,j,k),char(10)
    !         endif
    !      enddo
    !   enddo
    !enddo
